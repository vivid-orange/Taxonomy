...
{
    public class MemberDesign6_10CombinationTests
    {
        public static IList<IMemberDesignCombination> Create()
        {
            ...
            return ENCombinationFactory.CreateStrGeoSetB(loadCases, NationalAnnex.UnitedKingdom, false);
        }
        ...
        public void NameAndDefinitionTest()
        {
            ...
            // Assert
            Assert.Equal("LC1: STR/GEO Set B, Eq. 6.10 - Leading Live loads Category B",
                combinations[0].Name);
            Assert.Equal("" + new Ratio(1.35, RatioUnit.DecimalFraction) + "·(DL + SDL) + " + new Ratio(1.5, RatioUnit.DecimalFraction) + "·Q + " + new Ratio(1.5, RatioUnit.DecimalFraction) + "·(" + new Ratio(0.6, RatioUnit.DecimalFraction) + "·W + " + new Ratio(0.5, RatioUnit.DecimalFraction) + "·S + " + new Ratio(0.6, RatioUnit.DecimalFraction) + "·T)", combinations[0].Definition);

            Assert.Equal("LC2: STR/GEO Set B, Eq. 6.10 - Leading Wind loads",
                combinations[1].Name);
            Assert.Equal("" + new Ratio(1.35, RatioUnit.DecimalFraction) + "·(DL + SDL) + " + new Ratio(1.5, RatioUnit.DecimalFraction) + "·W + " + new Ratio(1.5, RatioUnit.DecimalFraction) + "·(" + new Ratio(0.7, RatioUnit.DecimalFraction) + "·Q + " + new Ratio(0.5, RatioUnit.DecimalFraction) + "·S + " + new Ratio(0.6, RatioUnit.DecimalFraction) + "·T)", combinations[1].Definition);

            Assert.Equal("LC3: STR/GEO Set B, Eq. 6.10 - Leading Wind loads with favourable permanent",
                combinations[2].Name);
            Assert.Equal("DL + SDL + " + new Ratio(1.5, RatioUnit.DecimalFraction) + "·W", combinations[2].Definition);

            Assert.Equal("LC4: STR/GEO Set B, Eq. 6.10 - Leading Snow loads",
                combinations[3].Name);
            Assert.Equal("" + new Ratio(1.35, RatioUnit.DecimalFraction) + "·(DL + SDL) + " + new Ratio(1.5, RatioUnit.DecimalFraction) + "·S + " + new Ratio(1.5, RatioUnit.DecimalFraction) + "·(" + new Ratio(0.7, RatioUnit.DecimalFraction) + "·Q + " + new Ratio(0.6, RatioUnit.DecimalFraction) + "·W + " + new Ratio(0.6, RatioUnit.DecimalFraction) + "·T)", combinations[3].Definition);

            Assert.Equal("LC5: STR/GEO Set B, Eq. 6.10 - Leading Thermal loads",
                combinations[4].Name);
            Assert.Equal("" + new Ratio(1.35, RatioUnit.DecimalFraction) + "·(DL + SDL) + " + new Ratio(1.5, RatioUnit.DecimalFraction) + "·T + " + new Ratio(1.5, RatioUnit.DecimalFraction) + "·(" + new Ratio(0.7, RatioUnit.DecimalFraction) + "·Q + " + new Ratio(0.6, RatioUnit.DecimalFraction) + "·W + " + new Ratio(0.5, RatioUnit.DecimalFraction) + "·S)", combinations[4].Definition);
        }
        ...
        public void FactoredLoadsTest()
        {
            ...
            // Assert
            // LC1: 1.35·(DL + SDL) + 1.5·Q + 1.5·(0.6·W + 0.5·S + 0.6·T)
            int i = 0;
            ...
            // LC2: 1.35·(DL + SDL) + 1.5·W + 1.5·(0.7·Q + 0.5·S + 0.6·T)
            i = 0;
            TestFactoredLoad(lc2Loads[i++], 1.35, dl);
            TestFactoredLoad(lc2Loads[i++], 1.35, sdl);
            ...
            // LC4: 1.35·(DL + SDL) + 1.5·S + 1.5·(0.7·Q + 0.6·W + 0.6·T)
            i = 0;
            TestFactoredLoad(lc4Loads[i++], 1.35, dl);
            TestFactoredLoad(lc4Loads[i++], 1.35, sdl);
            ...
            // LC5: 1.35·(DL + SDL) + 1.5·T + 1.5·(0.7·Q + 0.6·W + 0.5·S)
            i = 0;
            TestFactoredLoad(lc5Loads[i++], 1.35, dl);
            TestFactoredLoad(lc5Loads[i++], 1.35, sdl);
